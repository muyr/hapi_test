...
class MParameterPathFile(MParameterWidget):
    signal_path_file_parm_update = Signal(str)

    def __init__(self, parm_id, parm_label, path_file, parent=None):
        super(MParameterPathFile, self).__init__(parm_id, parent)
        ...
        if path_file:
            self.line_edit.setText(path_file)
            self.button.set_dayu_path(os.path.dirname(path_file))
